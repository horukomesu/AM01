...
def next_image():
    total = len(main_window.images)
    if total == 0:
        return
    idx = (getattr(main_window, "current_image_index", 0) + 1) % total
...
def add_locator():
    ...
    if getattr(main_window, "locator_mode", False):
        exit_locator_mode()
    sel_name = getattr(main_window, "selected_locator", None)
    if sel_name:
        loc = next((l for l in main_window.locators if l["name"] == sel_name), None)
        if loc:
            ...
            QtWidgets.QMessageBox.information(
                main_window,
                "Move Locator",
                f"Placing {sel_name}. Click on images to update its position. Press Esc to finish.",
            )
            return
